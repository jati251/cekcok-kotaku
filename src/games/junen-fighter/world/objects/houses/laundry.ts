...
export function buildLaundryHouse(ctx: WorldContext, p: Property) {
  ...
  const { wallDefault, roofGrey, white, dark, wood, concrete } = materials;
  ...
  buildAwning(ctx, w - 0.2, front + 0.12, 2.85, front, roofGrey, dark);

  // Indonesian warung / laundry signage
  sign('JUAL\nPULSA ELEKTRIK\n& LAUNDRY', 1.9, 2.3, -0.1, 1.25, 0.85);

  // Front Fence
  buildFence(ctx, w, 'laundry', m);
}
