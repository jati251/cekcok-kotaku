  ...
  const renderHeroCard = (heroType: HeroType) => {
    ...
    return (
      <motion.button
        ...
        className="group relative flex flex-col items-start bg-slate-900/90 hover:bg-slate-800/90 border border-slate-700/80 hover:border-amber-400 p-5 rounded-2xl transition-colors shadow-xl text-left cursor-pointer overflow-hidden"
      >
        <div
      ...
      <AnimatePresence mode="wait">
        ...
        {status === GameStatus.MENU && selectionStep === 'MAIN' && (
          ...
          >
            <motion.div
              ...
              className="inline-flex p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-400 mb-2"
            >
        ...
        {status === GameStatus.MENU && selectionStep === 'SCENARIO_SELECT' && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -20 }}
            className="w-full max-w-5xl space-y-6 max-h-[90vh] overflow-y-auto pr-1"
          >
              ...
              {DIFFICULTY_ORDER.map((diff) => {
                ...
                return (
                  <motion.button
                    ...
                    className="group flex items-center justify-between p-4 rounded-xl bg-slate-900/80 hover:bg-slate-800/90 border border-slate-800 hover:border-amber-400 transition-colors text-left cursor-pointer"
                  >
        ...
        {status === GameStatus.VICTORY && (
          ...
          >
            <motion.div
              ...
              className="inline-flex p-4 rounded-2xl bg-amber-500/10 text-amber-400"
            >
        ...
        {status === GameStatus.DEFEAT && (
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.85 }}
            className="text-center max-w-md bg-slate-900/95 border border-rose-600/50 p-8 rounded-2xl shadow-2xl space-y-5"
          >
